...
def analyze_logs(csv_file, reference_vm_id=1):
    ...
    # (A) Extract logical clock values using new log format:
    # Expected format: "CLOCK_UPDATE: <value>; EVENT: ..."
    ...
    # We'll look for "QUEUE: <number>" in the received messages log lines.
    ...
    def categorize_event(event_str):
        if "EVENT: INTERNAL" in event_str:
            return "Internal"
        elif "EVENT: SENT" in event_str:
            return "Sent"
        elif "EVENT: RECEIVED" in event_str:
            return "Received"
        elif "EVENT: SENT_DIRECT" in event_str:
            return "SentDirect"
        else:
            return "Other"
    ...
    # Drop rows without clock values for clock plots.
